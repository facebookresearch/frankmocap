...
def load_bbox_info(bbox_info_dir):
    ...
    for file in bbox_info_files:
        ...
        for key in one_hand_info:
            record = key.split('_')
            hand_type = '_'.join(record[-2:])
            person_id = record[-3]
            img_id = '_'.join(record[:-3])
            person_key_hand = f'{person_id}_{hand_type}' # 00_right_hand
            bbox_info[img_id][person_key_hand] = one_hand_info[key]
    return bbox_info
...
def load_pred_info(pred_info_file):
    ...
    for data in all_data:
        ...
        record = img_name.split('/')[-1].split('.')[0].split('_')
        hand_type = '_'.join(record[-2:])
        ...
        if hand_type == 'left_hand':
            ...
            pred_cam[1] *= -1 # flip x
        pred_info[img_id][person_key_hand] = dict(
            pred_cam = pred_cam,
            pred_pose = pred_pose)
    return pred_info
...
def load_all_data(root_dir):
    ...
    pred_info_file = osp.join(root_dir, "prediction/h3dw/pred_results.pkl")
...
def render_two_hand(frame_info, bbox_info, pred_info, res_dir, select_keys):
    ...
    for key_id, img_key in enumerate(sorted_keys):
        ...
        for person_key_hand in pred_info[img_key]:
            # print(img_key, person_key_hand)
            hand_type = '_'.join(person_key_hand.split('_')[-2:])
            assert hand_type in ['left_hand', 'right_hand']
            pred = pred_info[img_key][person_key_hand]
            ...
            bbox = bbox_info[img_key][person_key_hand]
