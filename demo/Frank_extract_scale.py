...
def scale_joints(output_json,body_bbox_list,height,width,width_n,height_n):
    size = len(output_json['posenet']['nose']['x'])
    scale_x = body_bbox_list[2]/2
    scale_y = body_bbox_list[3]/2
    scale_xn = width_n/width
    scale_yn = height_n/height
    xo = body_bbox_list[0] - body_bbox_list[2]
    yo = body_bbox_list[1]
    for key in output_json.keys():
        for little_key in output_json[key].keys():
          output_json[key][little_key]['x'][size-1] = ((output_json[key][little_key]['x'][size-1] + 1.0)*scale_x + xo)*scale_xn
          output_json[key][little_key]['y'][size-1] = ((output_json[key][little_key]['y'][size-1] + 1.0)*scale_y + yo)*scale_yn
          output_json[key][little_key]['z'][size-1] = (output_json[key][little_key]['z'][size-1] + 1.0)*(scale_x+scale_y)/2
    return output_json
...
def run_frank_mocap(args, bbox_detector, body_mocap, hand_mocap, visualizer):
    ...
    while True:
        ...
        output_json = scale_joints(output_json,body_bbox_list[0],height,width,width_n,height_n)
